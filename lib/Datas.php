...
class Datas
{
    ...
    {
        self::ckeckdate($dateFormat, $dateStr);
     ...
     * @return bool
     */
    static function ckeckdate(string $format, string $datestr, $throwException = true): bool
    {
        ...
        if (! checkdate($month, $day, $year)) {
            if ($throwException) {
                throw new Exception("Data incorreta ou inexistente ($datestr).");
            } else {
                return true;
            }
        } else {
     ...
     * @return string
     */
    static function staticGetWeekDayName(int $dayNumber, $fullName = false,$utf8_decode=false): string
    {
    ...
    {        
        ...
        return self::staticGetWeekDayName($dayNumber,$fullName);
    }
     ...
     * @return float
     */
    static function staticGetWeekDayNumber(float $timestamp): float
    {
    ...
    static function getMonthName(int $monthNumber, $fullName = true): string
    {
        if ($monthNumber < 1 || $monthNumber > 12) {
            throw new \Exception("Número inadequado para um mês ('$dayNumber').");
        }
        $code = $fullName ? '%B' : '%b';
    ...
    {
        return date('t', self::mktime('Ymd', $year.$month."01"));
    }
